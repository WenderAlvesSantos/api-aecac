...
async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    try {
      ...
      // Buscar apenas na collection de associados
      const user = await db.collection('users_associados').findOne({ 
    ...
    try {
      ...
      // Atualizar na collection de associados
      await db.collection('users_associados').updateOne(
        { _id: new ObjectId(req.userId) },
        { $set: updateData }
      )

      const updatedUser = await db.collection('users_associados').findOne({ 
        _id: new ObjectId(req.userId) 
      })
