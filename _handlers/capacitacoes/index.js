  ...
  if (req.method === 'GET') {
    try {
      ...
      if (token) {
        try {
          ...
          if (decoded) {
            ...
            console.log('🔍 [CAPACITACOES] userInfo:', {
          ...
          console.error('❌ [CAPACITACOES] Erro ao verificar token:', error)
      ...
      if (userInfo && !userInfo.isAssociado && userInfo.isAdmin) {
        console.log('✅ [CAPACITACOES] Admin detectado - filtrando apenas itens sem empresaId')
        query.$and = [
          {
            $or: [
              { ativo: true },
